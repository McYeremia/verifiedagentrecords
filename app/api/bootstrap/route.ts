...
export async function GET(req: NextRequest) {
  ...
  // [RESULT] records are surfaced inside recallUserMemories itself (a dedicated
  // RESULT-focused recall merged into the shared cache), so `memories` is already
  // complete here — no secondary recall needed. Without complete results the
  // predict sidebar would show resolved matches as "pending".
